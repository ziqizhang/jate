...
public class ParallelIndexingMaster {
    ...
    public static void main(String[] args) {
        SolrClient solrClient =
                new EmbeddedSolrServer(Paths.get("D:\\Work\\jate_github\\jate\\solr-5.3.0\\server\\solr"), JATEnum.SOLR_CORE_NAME.getString());
        ParallelIndexingMaster m = new ParallelIndexingMaster();
        List<String> files = new ArrayList<>();
        for(File f: new File("D:\\Work\\jate_github\\jate\\sample\\input").listFiles())
            files.add(f.toString());
        m.index(files, 10,5,new TikaSimpleDocumentCreator(),solrClient, 0.5);
    }
}
