...
import uk.ac.shef.dcs.jate.algorithm.Relevance;
...
/**
 * Created by zqz on 22/05/17.
 */
public class AppRelevance extends App {
    private final Logger log = LoggerFactory.getLogger(AppRelevance.class.getName());
    ...
    public static void main(String[] args) {
        ...
        List<JATETerm> terms;
        try {
            App relevance = new AppRelevance(params);
            if (isCorpusProvided(corpusDir)) {
                relevance.index(Paths.get(corpusDir), Paths.get(solrHomePath), solrCoreName, jatePropertyFile);
            }

            terms = relevance.extract(solrHomePath, solrCoreName, jatePropertyFile);

            if (isExport(params)) {
                relevance.write(terms);
            }
     ...
     * @see AppParams#REFERENCE_FREQUENCY_FILE
     */
    public AppRelevance(Map<String, String> initParams) throws JATEException {
        super(initParams);
    ...
    public List<JATETerm> extract(SolrCore core, JATEProperties properties) throws JATEException {
        ...
        try {
            ...
            relevance.registerFeature(FrequencyTermBased.class.getName() + Weirdness.SUFFIX_WORD, fwb);
            relevance.registerFeature(FrequencyTermBased.class.getName() + Weirdness.SUFFIX_REF, frb);

            List<String> candidates = new ArrayList<>(this.freqFeature.getMapTerm2TTF().keySet());

            filterByTTF(candidates);

            List<JATETerm> terms = relevance.execute(candidates);
            terms = cutoff(terms);
    ...
    protected static void printHelp() {
        StringBuilder sb = new StringBuilder("Relevance Usage:\n");
        sb.append("java -cp '[CLASSPATH]' ").append(AppRelevance.class.getName()).append(" [OPTIONS] ")
