...
public class FrequencyCtxWindowBasedFBMaster extends AbstractFeatureBuilder{
    ...
    private int window;
    private Map<Integer, List<Context>> contextLookup;

    /**
     *
     * @param solrIndexSearcher
     * @param properties
     * @param existingContexts if we want to use context windows already generated by another process
    ...
    public FrequencyCtxWindowBasedFBMaster(SolrIndexSearcher solrIndexSearcher,
                                           ...
                                           Set<Context> existingContexts,
                                           int window, int termOrWord) {
        super(solrIndexSearcher, properties);
        this.termOrWord=termOrWord;
        this.window=window;
        for(Context ctx: existingContexts){
            List<Context> container = contextLookup.get(ctx.getDocId());
            if(container==null)
